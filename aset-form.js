        ...
        t_onReady(function () {
            setTimeout(function () {
                window.t_upwidget__init
                    ? t_zeroForms__onFuncLoad("t_upwidget__init", () => btn.classList.remove("t-submit"))
                    : btn.classList.remove("t-submit")
            }, 500)
        })
    };

    // Ожидание прохождения Yandex ReCaptcha
    function waitForCaptcha(form, callback) {
        const widgetContainer = form.querySelector('.t-upwidget');

        if (!widgetContainer) {
            console.warn('[TKFORM] Нет .t-upwidget в форме — капча не требуется');
            callback();
            return;
        }

        if (widgetContainer.classList.contains('t-upwidget__success')) {
            callback();
            return;
        }
